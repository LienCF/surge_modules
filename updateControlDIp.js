...
(async function() {
   ...
   await ControlDPreAuth();
   await ControlDLogin($.TOKEN);
   await ControlDGetUsers($.SESSION);
...
function ControlDLogin(token) {
    return $.http.post({
            ...
            body: `{\"email\":\"chihfeng.lien@xamide.com\",\"password\":\"roum6phaw-krax-GEAW\",\"token\":\"${token}\"}`
        })
        .then(async (resp) => {
            const body = JSON.parse(resp.body);
            $.log(resp.body);
...
function ControlDGetUsers(session) {
    return $.http.get({
